...
- OpenClaw behind the `*.lolgames.net` tunnel needs proxy-aware gateway config. Keep loopback proxies trusted (`127.0.0.1/32`, `::1/128`), enable `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true`, and populate `gateway.controlUi.allowedOrigins` with the exact public OpenClaw origin derived from `~/.worker-agents/state.json`. A literal `["*"]` was not sufficient in testing on July 22, 2026; the Control UI websocket still failed with `Browser origin not allowed` until the exact `http://<worker-host>:18789` origin was present.
