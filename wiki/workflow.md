...
- OpenClaw behind the `*.lolgames.net` tunnel needs proxy-aware gateway config. Keep loopback proxies trusted (`127.0.0.1/32`, `::1/128`) and enable `gateway.controlUi.dangerouslyAllowHostHeaderOriginFallback=true`; otherwise the Control UI can fail with `Browser origin not allowed` because the tunnel proxy reaches OpenClaw over loopback without forwarded origin headers.
